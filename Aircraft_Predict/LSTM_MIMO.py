...
# 实例化模型
lstm_long = LSTM(input_size, hidden_size, num_layers, output_size).cuda()
lstm_lat = LSTM(input_size, hidden_size, num_layers, output_size).cuda()
lstm_alt = LSTM(input_size, hidden_size, num_layers, output_size).cuda()

# 定义损失函数和优化器
criterion = nn.MSELoss()
optimizer_long = torch.optim.Adam(lstm_long.parameters(), lr=learning_rate)
optimizer_lat = torch.optim.Adam(lstm_lat.parameters(), lr=learning_rate)
optimizer_alt = torch.optim.Adam(lstm_alt.parameters(), lr=learning_rate)
...
for epoch in range(num_epochs):
    # 训练经度模型
    lstm_long.train()
    outputs_long = lstm_long(trainX_long)
    ...
    # 训练纬度模型
    lstm_lat.train()
    outputs_lat = lstm_lat(trainX_lat)
    ...
    # 训练地理高度模型
    lstm_alt.train()
    outputs_alt = lstm_alt(trainX_alt)
...
# 模型评估
lstm_long.eval()
train_predict_long = lstm_long(dataX_long).cpu().data.numpy()
...
lstm_lat.eval()
train_predict_lat = lstm_lat(dataX_lat).cpu().data.numpy()
...
lstm_alt.eval()
train_predict_alt = lstm_alt(dataX_alt).cpu().data.numpy()
...
# 绘制真实轨迹
ax.plot(dataY_plot[:, 0], dataY_plot[:, 1], dataY_plot[:, 2], label='True Trajectory', color='b')

# 绘制预测轨迹
ax.plot(data_predict[:train_size, 0], data_predict[:train_size, 1], data_predict[:train_size, 2], label='Predicted Trajectory (Train)', color='g')

ax.plot(data_predict[train_size:, 0], data_predict[train_size:, 1], data_predict[train_size:, 2], label='Predicted Trajectory (Test)', color='r')
